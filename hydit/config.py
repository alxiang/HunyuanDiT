...
def model_var_type(value):
    try:
        ...
        raise ValueError(
            f"Invalid choice '{value}', valid choices are {[v.name for v in ModelVarType]}"
        )
...
def get_args(default_args=None):
    ...
    parser.add_argument(
        "--seed", type=int, default=42, help="A seed for all the prompts."
    )
    ...
    parser.add_argument(
        "--extra-fp16",
        action="store_true",
        help="Use extra fp16 for vae and text_encoder.",
    )

    # HunYuan-DiT
    parser.add_argument(
        "--model", type=str, choices=list(HUNYUAN_DIT_CONFIG.keys()), default="DiT-g/2"
    )
    parser.add_argument(
        "--image-size",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1024,
        help="Hidden size of CLIP text encoder.",
    )
    parser.add_argument(
        "--text-len",
        type=int,
        default=77,
        help="Token length of CLIP text encoder output.",
    )
    parser.add_argument(
        "--text-states-dim-t5",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=256,
        help="Token length of T5 text encoder output.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="+",
        default=["Wqkv", "q_proj", "kv_proj", "out_proj"],
    ...
    parser.add_argument(
        "--control-weight", type=float, default=1.0, help="Controlnet weight"
    )
    parser.add_argument(
        "--condition-image-path",
        type=str,
        default=None,
        help="Inference condition image path",
    )

    # Diffusion
    parser.add_argument(
        "--learn-sigma", action="store_true", help="Learn extra channels for sigma."
    )
    ...
    parser.add_argument(
        "--beta-start", type=float, default=0.00085, help="Beta start value"
    )
    ...
    parser.add_argument(
        ...
        type=model_var_type,
        default=None,
        help="Specify the model variable type.",
    )
    parser.add_argument(
        "--noise-offset",
        type=float,
        default=0.0,
        help="Add extra noise to the input image.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="一只小猫",
        help="The prompt for generating images.",
    )
    parser.add_argument(
        "--model-root", type=str, default="ckpts", help="Model root path."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=6.0,
        help="Guidance scale for classifier-free.",
    )
    ...
    parser.add_argument(
        "--onnx-workdir", type=str, default="onnx_model", help="Path to save ONNX model"
    )
    ...
    # Prompt enhancement
    parser.add_argument(
        "--enhance", action="store_true", help="Enhance prompt with dialoggen."
    )
    parser.add_argument("--no-enhance", dest="enhance", action="store_false")
    parser.add_argument(
        "--load-4bit",
        help="load DialogGen model with 4bit quantization.",
        action="store_true",
    )
    ...
    # App
    parser.add_argument(
        "--lang", type=str, default="zh", choices=["zh", "en"], help="Language"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=100_000,
        help="Create a ckpt every a few steps.",
    )
    parser.add_argument(
        "--ckpt-latest-every",
        type=int,
    ...
    parser.add_argument(
        "--weight-decay", type=float, default=0, help="weight-decay in optimizer"
    )
    parser.add_argument(
        "--rope-img",
        type=str,
    ...
    parser.add_argument(
        "--resume", type=str, default=None, help="Resume experiment from a checkpoint"
    )
    parser.add_argument(
        "--no-strict",
        dest="strict",
        action="store_false",
        help="Strict loading of checkpoint",
    )
    ...
    parser.add_argument(
        "--multireso", action="store_true", help="Use multi-resolution training."
    )
    parser.add_argument(
        "--reso-step",
        type=int,
        default=None,
        help="Step size for multi-resolution training.",
    )

    # Additional condition
    parser.add_argument(
        "--random-shrink-size-cond",
        action="store_true",
    ...
    parser.add_argument(
        "--merge-src-cond",
        action="store_true",
        help="Merge the source condition into a single value.",
    )
    ...
    parser.add_argument(
        "--use-flash-attn",
        action="store_true",
        help="During training, " "flash attention is used to accelerate training.",
    )
    parser.add_argument(
        "--no-flash-attn",
        dest="use_flash_attn",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Use AngelPTM zero stage. Support 2 and 3",
    )
    parser.add_argument(
        "--grad-accu-steps", type=int, default=1, help="Gradient accumulation steps."
    )
    ...
    parser.add_argument(
        "--async-ema",
        action="store_true",
        help="Whether to use multi stream to excut EMA.",
    )
