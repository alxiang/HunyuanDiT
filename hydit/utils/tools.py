...
def get_trainable_params(model):
    ...
    params = [
        ...
        if not name.startswith("module.text_encoder")
        and not name.startswith("module.text_encoder_t5")
    ]
    ...
    text_encoder_params = [
        ...
        or name.startswith("module.text_encoder_t5")
    ]
...
def model_resume(args, model, ema, logger, len_loader):
    ...
    logger.info(
        f"    Resume deepspeed={args.resume_deepspeed}, "
    ...
    if args.resume_deepspeed:
        assert "mp_rank_00_model_states.pt" in os.listdir(
            resume_path
        ), f"    Cannot find dp chkpt from {resume_path}"
        resume_ckpt = torch.load(
            os.path.join(resume_path, "mp_rank_00_model_states.pt"),
    ...
    # Resume model and ema states (not include optimizer states) from two checkpoints separated from DeepSpeed ckpt.
        ...
        if args.ema_to_module:
            assert "pytorch_model_ema.pt" in os.listdir(
                resume_path
            ), f"    Cannot find pytorch_model_ema.pt from {resume_path}"
            logger.info(f"    Resume main model from ema states.")
            resume_ckpt_ema = torch.load(
                os.path.join(resume_path, "pytorch_model_ema.pt"),
                map_location=lambda storage, loc: storage,
            )
            ...
            assert "pytorch_model_module.pt" in os.listdir(
                resume_path
            ), f"    Cannot find pytorch_model_module.pt from {resume_path}"
            logger.info(f"    Resume main model from main states.")
            resume_ckpt_module = torch.load(
                os.path.join(resume_path, "pytorch_model_module.pt"),
                map_location=lambda storage, loc: storage,
            )
        ...
        raise ValueError(
            "    “If `resume` is True, then either `resume_split` or `resume_deepspeed` must be true.”"
        )
